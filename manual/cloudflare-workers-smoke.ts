...
//   IRIS_EDGE_SMOKE=1 node manual/cloudflare-workers-smoke.ts
//
...
// `miniflare` (the local workerd) installed AND the @iris/core + @iris/store-do ESM
...
// setAlarm/getAlarm) that `@iris/store-do` was written against — the adapter below is
...
// (@iris/store-do/src/do-storage.ts). Values are Uint8Array (DO storage is
...
async function main() {
  ...
  try {
    ...
    console.error(
      "cloudflare-workers-smoke: `miniflare` is not installed (future edge target). " +
        "Install it (npm i -D miniflare) AND bundle @iris/core + @iris/store-do into the Worker " +
  ...
  // (Bundling note: miniflare must resolve @iris/* — configure modulesRoot or a
  ...
  const script = `
    ...
    export class AgentDO {
      ...
      async fetch(req) {
        ...
        const { runTurnOn } = await import("@iris/host");
