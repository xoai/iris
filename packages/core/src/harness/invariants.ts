// Kernel-enforced invariants (spec §3.6). Two DISTINCT mechanisms (do not conflate):
//
...
// only TIGHTEN, never loosen them. (Real egress enforcement lands in M3 with a
// network boundary; in M2 it is a constant/type-level default.)
import type { HarnessState, Phase } from "./kernel.ts";

export interface Invariants {
  // Hard cap on loop steps (1 step per effect) — bounds every runaway. A "turn"
  // is the whole loop from initial state to `finish`; in M2 that spans the entire
  ...
  // unbounded park/resume loop. When real multi-turn sessions land (M5), a new
  ...
  egressDefault: "deny-all"; // pinned: M2 has no runtime network (enforced for real in M3)
}
